...
async def ask_agent_stream(req: QueryRequest):
    ...
    async def event_generator():
        try:
            ...
            for chunk in ollama_client.chat(model=OLLAMA_MODEL, messages=[{'role': 'user', 'content': prompt}], stream=True):
...
async def upload_image_stream(file: UploadFile = File(...), language: str = Form("hi"), query: str = Form("")):
